...
public partial class RandB_page_펀치 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            sourceControl.Src = "~/RandB music/펀치 (Punch)-01-밤이 되니까.mp3";
            string connectionString = @"server=(localdb)\MSSQLLocalDB;Integrated Security=true; database=PALYLIST_USER";
            SqlConnection Con = new SqlConnection(connectionString);
            SqlCommand Cmd = new SqlCommand();
            Cmd.Connection = Con;
            Cmd.CommandText = "SELECT * FROM punch";
            Con.Open();
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        if (Session["id"] == null)
        {
            ...
            Cmd_1.CommandText = "SELECT * FROM punch";
            Con_1.Open();
        ...
        else
        {
            ...
            else
            {
                ...
                SqlCommand Cmd = new SqlCommand();
                Cmd.Connection = Con;
                Cmd.CommandText = "INSERT INTO punch (comment) VALUES " + "( N'" + TextBox1.Text + "' )";
                Con.Open();
