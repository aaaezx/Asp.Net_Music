...
public partial class hiphap_page_jessi : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            sourceControl.Src = "~/hiphap music/제시-눈누난나.mp3";
            sourceControl1.Src = "~/hiphap music/who dat b.mp3";
            string connectionString = @"server=(localdb)\MSSQLLocalDB;Integrated Security=true; database=PALYLIST_USER";
            SqlConnection Con = new SqlConnection(connectionString);
            SqlCommand Cmd = new SqlCommand();
            Cmd.Connection = Con;
            Cmd.CommandText = "SELECT * FROM jessi";
            Con.Open();
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        if (Session["id"] == null)
        {
            ...
            Cmd_1.CommandText = "SELECT * FROM jessi";
            Con_1.Open();
        ...
        else
        {
            ...
            else
            {
                ...
                SqlCommand Cmd = new SqlCommand();
                Cmd.Connection = Con;
                Cmd.CommandText = "INSERT INTO jessi (comment) VALUES " + "( N'" + TextBox1.Text + "' )";
                Con.Open();
