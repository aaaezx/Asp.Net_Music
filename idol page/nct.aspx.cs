...
public partial class idol_page_nct : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            sourceControl.Src = "~/idol music/NCT U-01-Make A Wish (Birthday Song).mp3";
            string connectionString = @"server=(localdb)\MSSQLLocalDB;Integrated Security=true; database=PALYLIST_USER";
            SqlConnection Con = new SqlConnection(connectionString);
            SqlCommand Cmd = new SqlCommand();
            Cmd.Connection = Con;
            Cmd.CommandText = "SELECT * FROM nct";
            Con.Open();
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        if (Session["id"] == null)
        {
            ...
            Cmd_1.CommandText = "SELECT * FROM nct";
            Con_1.Open();
        ...
        else
        {
            ...
            else
            {
                ...
                SqlCommand Cmd = new SqlCommand();
                Cmd.Connection = Con;
                Cmd.CommandText = "INSERT INTO nct (comment) VALUES " + "( N'" + TextBox1.Text + "' )";
                Con.Open();
